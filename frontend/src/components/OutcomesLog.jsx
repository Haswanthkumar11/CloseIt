...
export function OutcomesLog() {
  ...
  const fetchOutcomes = async () => {
    ...
    try {
      ...
      setError('Could not load outcomes log.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          <h2 style={{ fontSize: '1.75rem', fontWeight: 800 }}>Merchant Rescue Outcomes Log (/outcomes)</h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
            Real-time conversion metrics and objection resolution history fetched from GET /outcomes.
          </p>
        </div>
      ...
      <div className="glass-panel" style={{ padding: '1.5rem', overflow: 'hidden' }}>
        <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '1rem' }}>Recorded Outcomes History (Newest First)</h3>
